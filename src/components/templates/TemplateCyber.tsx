...
import { Home, User, Briefcase, FileText, ImageIcon, Mail, ChevronRight, ExternalLink, Code2 } from "lucide-react";
...
export default function TemplateCyber({
  ...
  const subtitle = heroSection?.content?.subtitle || "Full-stack developer ready for new deployments.";
  const avatarUrl =
    portfolio.logoUrl ||
  ...
  const getSkillLevel = (s: any): number | null => {
    if (s && typeof s === "object" && s.level != null) return Number(s.level);
    return null;
  };

  const getTools = (tools: any): string[] => {
    if (!tools) return [];
    if (typeof tools === "string") return tools.split(",").map((t: string) => t.trim()).filter(Boolean);
  ...
  return (
    <div
      ...
      style={{ backgroundColor: secondaryColor, fontFamily: portfolio.fontFamily }}
    >
      ...
      <header className="fixed top-6 left-0 right-0 z-50 flex justify-center pointer-events-none">
        <div className="flex items-center gap-1 md:gap-2 bg-gray-950/80 backdrop-blur-xl border border-gray-800/60 p-1.5 rounded-full pointer-events-auto shadow-2xl shadow-black/50 overflow-x-auto max-w-[95%] md:max-w-none">
          ...
          <button className="flex items-center gap-2 px-4 py-2 bg-gray-800/80 rounded-full text-white text-sm font-medium border border-gray-700/50 shrink-0">
            <User className="w-4 h-4" /> <span className="hidden sm:inline">About</span>
          </button>
          <button className="flex items-center gap-2 px-4 py-2 text-gray-400 hover:text-white hover:bg-gray-800/40 rounded-full text-sm font-medium transition-colors shrink-0">
            <Briefcase className="w-4 h-4" /> <span className="hidden sm:inline">Work</span>
          </button>
          <button className="flex items-center gap-2 px-4 py-2 text-gray-400 hover:text-white hover:bg-gray-800/40 rounded-full text-sm font-medium transition-colors shrink-0">
            <FileText className="w-4 h-4" /> <span className="hidden sm:inline">Projects</span>
          </button>
        </div>
      </header>
        ...
        <aside className="lg:w-64 lg:fixed lg:h-screen flex flex-col justify-center px-8 z-40 lg:border-r border-gray-800/30">
          <nav className="space-y-8 text-sm font-medium text-gray-500">
            {[
              ...
              { href: "#projects", label: "Selected Projects", active: false },
              { href: "#work", label: "Work Experience", active: false },
              { href: "#skills", label: "Technical skills", active: false },
            ].map(({ href, label, active }) => (
              <a key={href} href={href} className={`flex items-center gap-4 transition-colors group ${active ? "text-gray-200" : "hover:text-gray-200"}`}>
                <span className="w-4 h-px transition-colors" style={{ backgroundColor: active ? primaryColor : "rgb(31 41 55)" }} />
                {label}
              </a>
            ))}
          </nav>
        ...
        <main className="flex-1 lg:ml-64 flex py-10 lg:py-40 px-6 lg:px-20 min-h-screen">
          <div className="flex flex-col gap-16 lg:gap-24 w-full">
            ...
            <div className="flex flex-col md:flex-row gap-16 lg:gap-24 w-full justify-center">
              ...
              >
                <div className="relative w-48 h-48 md:w-64 md:h-64 rounded-full p-1 shadow-2xl mb-8 group" style={{ background: "linear-gradient(to top right, rgb(31 41 55), rgb(55 65 81), rgb(75 85 99))" }}>
                  <div className="w-full h-full rounded-full overflow-hidden border-4 relative z-10" style={{ borderColor: secondaryColor }}>
                    <img src={avatarUrl} alt="Portrait" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                  </div>
                </div>
                <div className="flex flex-col items-center space-y-3">
                  ...
                  {portfolio.profession && (
                    <div className="flex items-center gap-2 text-sm text-gray-400 font-mono">
                      <Briefcase className="w-4 h-4" /> {String(portfolio.profession)}
                    </div>
                  )}
                </div>
              </motion.div>
              ...
              >
                ...
                <div id="intro" className="space-y-2 mb-8">
                  <h1 className="text-5xl md:text-7xl font-bold tracking-tight text-white mb-2">{String(title)}</h1>
                  <h2 className="text-2xl md:text-4xl font-light" style={{ color: primaryColor }}>
                ...
                <div className="flex flex-wrap gap-3 mb-12">
                  {portfolio.socialLinks?.linkedin && (
                    <a href={String(portfolio.socialLinks.linkedin)} target="_blank" rel="noopener noreferrer"
                  ...
                  {portfolio.socialLinks?.twitter && (
                    <a href={String(portfolio.socialLinks.twitter)} target="_blank" rel="noopener noreferrer"
                      className="flex items-center gap-2 px-4 py-2 bg-gray-900 hover:bg-gray-800 border border-gray-800 rounded-xl text-sm transition-all">
                      <TwitterIcon className="w-4 h-4" /> Twitter
                    </a>
                  )}
                  {portfolio.socialLinks?.github && (
                    <a href={String(portfolio.socialLinks.github)} target="_blank" rel="noopener noreferrer"
                      className="flex items-center gap-2 px-4 py-2 bg-gray-900 hover:bg-gray-800 border border-gray-800 rounded-xl text-sm transition-all">
            ...
            {Array.isArray(portfolio.projects) && portfolio.projects.length > 0 && (
              <section id="projects" className="w-full mt-20">
                <div className="flex items-center gap-4 mb-12">
                  <h3 className="text-4xl font-bold text-white tracking-tighter">SELECTED PROJECTS</h3>
                      ...
                      <div className="relative z-10 flex flex-col gap-6">
                        <div className="flex justify-between items-start">
                          <div className="p-3 rounded-2xl bg-white/5 border border-white/10 w-fit">
                            <Code2 className="w-6 h-6 text-gray-300" style={{ color: primaryColor }} />
                          </div>
                          <span className="font-mono text-xs text-gray-500">0{idx + 1} / PROJECT</span>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                          <div className="space-y-4">
                            ...
                            <p className="text-gray-400 text-lg leading-relaxed max-w-xl">
                              {String(project.description || "Detailed analysis and implementation of system architecture and frontend components.")}
                            </p>
                          </div>
                          ...
                          <div className="flex flex-col gap-6">
                            <div className="flex flex-wrap gap-2">
                              {getTools(project.tools).map((t: string, j: number) => (
                                <span key={j} className="text-xs px-4 py-1.5 rounded-full bg-white/5 border border-white/5 text-gray-300 font-mono">
                                  {t}
                                </span>
                              ))}
                            </div>
                            ...
                            {project.link && (
                              ...
                              >
                                Explore Project <ExternalLink className="w-4 h-4 group-hover/btn:translate-x-1 group-hover/btn:-translate-y-1 transition-transform" />
                              </a>
                            )}
                          </div>
                        </div>
            ...
            {Array.isArray(portfolio.experience) && portfolio.experience.length > 0 && (
              <section id="work" className="mb-16 pt-20">
                <h3 className="text-3xl font-bold mb-10 text-white">Work Experience</h3>
                <div className="space-y-12">
                  {portfolio.experience.map((exp: any, idx: number) => (
                    <div key={idx} className="relative pl-8 md:pl-12 border-l border-gray-800">
                      <div className="absolute left-[-5px] top-2 w-2.5 h-2.5 rounded-full" style={{ backgroundColor: primaryColor }} />
                      <h4 className="text-2xl font-bold text-white">{String(exp.role || "")}</h4>
                      <p className="font-mono text-sm mt-1" style={{ color: primaryColor }}>{String(exp.company || "")} • {String(exp.period || "")}</p>
                      {exp.description && <p className="text-gray-400 mt-4 text-base leading-relaxed max-w-3xl">{String(exp.description)}</p>}
                    </div>
                  ))}
                </div>
              </section>
            ...
            {/* Education / Courses */}
            {Array.isArray(portfolio.courses) && portfolio.courses.length > 0 && (
              <section id="studies" className="mb-16">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {portfolio.courses.map((course: any, idx: number) => (
                    <div key={idx} className="bg-gray-900/50 backdrop-blur-sm rounded-2xl p-6 border border-gray-800 hover:border-gray-700 transition-colors">
                      <h4 className="font-bold text-white text-lg">{String(course.name || "")}</h4>
                      <p className="text-sm mt-2 opacity-80" style={{ color: primaryColor }}>{String(course.institution || "")} · {String(course.year || "")}</p>
                      {course.description && <p className="text-gray-400 text-sm mt-3 leading-relaxed">{String(course.description)}</p>}
                    </div>
                  ))}
                </div>
              </section>
            ...
            {Array.isArray(portfolio.skills) && portfolio.skills.length > 0 && (
              <section id="skills" className="mb-16">
                <h3 className="text-3xl font-bold mb-8 text-white">Skills</h3>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {portfolio.skills.map((skill: any, idx: number) => {
                    ...
                    if (!name) return null;
                    return (
                      <div key={idx} className="group p-4 bg-gray-900/40 border border-gray-800 rounded-2xl hover:border-cyan-500/50 transition-all text-center">
            ...
            <section className="mb-32">
              <div className="p-8 md:p-12 rounded-[2rem] bg-gradient-to-br from-gray-900 to-black border border-white/5 relative overflow-hidden text-center">
                <div className="relative z-10">
                  <h3 className="text-3xl md:text-5xl font-bold mb-6 text-white">Let's build something together</h3>
                  <p className="text-gray-400 mb-8 max-w-xl mx-auto text-lg">Available for freelance opportunities and full-time positions. System is online and ready for new input.</p>
                  <div className="flex flex-wrap justify-center gap-6 text-gray-300">
                    {portfolio.email && (
                      <a href={`mailto:${portfolio.email}`} className="text-xl font-medium hover:text-white transition-colors underline decoration-cyan-500/50 underline-offset-8">
                        {String(portfolio.email)}
                      </a>
                    )}
                  </div>
      ...
      <footer className="py-10 px-6 border-t border-gray-800/50 text-center text-gray-600 text-sm font-mono">
        &copy; {new Date().getFullYear()} — SYSTEM_STABLE — {String(title).toUpperCase()}
      </footer>
    </div>
  );
}
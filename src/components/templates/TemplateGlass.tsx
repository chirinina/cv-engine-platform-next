...
const AmbientBackground = ({ primaryColor, secondaryColor }: { primaryColor: string, secondaryColor: string }) => {
...
export default function TemplateGlass({
  ...
  const subtitle = heroSection?.content?.subtitle || "Designing interactive experiences through code and imagination.";
  ...
  return (
    ...
    >
      <AmbientBackground primaryColor={primaryColor} secondaryColor={secondaryColor} />
      ...
      >
        <div className="flex items-center gap-6 px-8 py-4 bg-white/5 backdrop-blur-2xl border border-white/10 shadow-2xl rounded-full">
          <a href="#home" className="text-sm font-medium hover:text-white text-slate-300 transition-colors">Home</a>
          <a href="#projects" className="text-sm font-medium hover:text-white text-slate-300 transition-colors">Projects</a>
          <a href="#experience" className="text-sm font-medium hover:text-white text-slate-300 transition-colors">Experience</a>
          <a href="#skills" className="text-sm font-medium hover:text-white text-slate-300 transition-colors">Skills</a>
        </div>
      </motion.nav>

      <main className="relative z-10 max-w-6xl mx-auto px-6 py-32 md:py-40 flex flex-col gap-32">
        {/* Hero Section */}
        <section id="home" className="flex flex-col lg:flex-row items-center gap-16 min-h-[70vh]">
            ...
            >
              ...
              <div className="flex gap-4 items-center pl-4 bg-white/5 border border-white/10 rounded-xl px-4 backdrop-blur-md">
                {portfolio.socialLinks?.linkedin && (
                  <a href={portfolio.socialLinks.linkedin} target="_blank" rel="noopener noreferrer" className="p-2 text-slate-400 hover:text-white transition-colors">
                    <LinkedinIcon className="w-5 h-5" />
                  </a>
                )}
                {portfolio.socialLinks?.github && (
                  <a href={portfolio.socialLinks.github} target="_blank" rel="noopener noreferrer" className="p-2 text-slate-400 hover:text-white transition-colors">
                    <GithubIcon className="w-5 h-5" />
                  </a>
                )}
                {portfolio.socialLinks?.twitter && (
                  <a href={portfolio.socialLinks.twitter} target="_blank" rel="noopener noreferrer" className="p-2 text-slate-400 hover:text-white transition-colors">
            ...
            <div className="aspect-[4/5] rounded-[3rem] overflow-hidden relative border border-white/20 shadow-2xl">
              ...
              {portfolio.profession && (
                ...
                >
                  ...
                  <span className="font-medium text-sm text-white">{portfolio.profession}</span>
                </motion.div>
              )}
              {portfolio.location && (
                <motion.div 
                ...
                >
                  ...
                  <span className="font-medium text-sm text-white">{portfolio.location}</span>
                </motion.div>
              )}
            </div>
          </motion.div>
        ...
        {portfolio.projects && portfolio.projects.length > 0 && (
          <section id="projects" className="space-y-12">
            <GlassCard delay={0}>
              ...
              <p className="text-slate-400 max-w-2xl text-lg mb-12">A selection of my recent projects, showcasing my ability to solve complex problems through design and code.</p>
                ...
                {portfolio.projects.map((project: any, idx: number) => (
                  ...
                  >
                    ...
                    <div className="space-y-6">
                      <h3 className="text-2xl font-bold text-white group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-white/70 transition-all">{project.name || project.title}</h3>
                      <p className="text-slate-400 leading-relaxed font-light line-clamp-3">{project.description}</p>
                      
                      <div className="flex flex-wrap gap-2 pt-2">
                        {getTools(project.tools).map((t: string, j: number) => (
                          <span key={j} className="text-xs px-3 py-1 rounded-lg bg-white/5 text-slate-300 border border-white/10">
                            {t}
                          </span>
                        ))}
                      </div>
              ...
              <div className="space-y-10">
                {portfolio.experience.map((exp: any, idx: number) => (
                  <div key={idx} className="relative pl-8 group">
                    <div className="absolute left-0 top-1.5 w-3 h-3 rounded-full border-2 border-slate-900 z-10" style={{ backgroundColor: secondaryColor }} />
                    {idx !== portfolio.experience.length - 1 && (
                    ...
                    <div className="space-y-2">
                      ...
                      <h4 className="text-lg font-medium" style={{ color: secondaryColor }}>{exp.company}</h4>
                      {exp.description && (
                ...
                {portfolio.courses.map((course: any, idx: number) => (
                  <div key={idx} className="p-6 rounded-2xl bg-white/5 border border-white/10 hover:border-white/20 transition-all">
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-2 mb-4">
                      <span className="text-[15px] font-medium" style={{ color: primaryColor }}>{course.institution}</span>
                      <span className="text-sm px-3 py-1 rounded-md bg-white/5 text-slate-400">{course.year}</span>
                    </div>
                    {course.description && (
                      <p className="text-slate-400 font-light leading-relaxed">{course.description}</p>
                    )}
                  </div>
                ))}
              </div>
        ...
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 rounded-full border-2 border-white/20 overflow-hidden">
              <img src={avatarUrl} alt="Footer avatar" className="w-full h-full object-cover" />
            </div>
            <div>
              <p className="font-bold text-white">{title}</p>
              <p className="text-xs text-slate-500 uppercase tracking-wider">{portfolio.profession}</p>
            </div>
          </div>
